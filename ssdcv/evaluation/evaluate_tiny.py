...
import sys
import shutil
from ssdcv.deps.Cityscapes.cityperson_eval import cityperson_eval
...
dependency install:
    cd ssdcv/deps/mini_maskrcnn_benchmark/ && python setup.py build develop

run example:
    python ssdcv/evaluation/evaluate_tiny.py --res exp/latest_result.json \
...
def merge_det_result(json_result_file, corner_gt_file, merged_gt_file, merge_nms_th=1.0):
    from ssdcv.corner_dataset.split_and_merge_image import COCOMergeResult
