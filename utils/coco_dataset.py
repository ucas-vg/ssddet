...
class CocoDataset:
    ...
    def get_ann_ids(self, img_ids=[], cat_ids=[], area_rng=[], iscrowd=None):
        return self.coco.getAnnIds(img_ids, cat_ids, area_rng, iscrowd)

    def get_cat_ids(self, cat_names=[], sup_names=[], cat_ids=[]):
        return self.coco.getCatIds(cat_names, sup_names, cat_ids)

    def get_img_ids(self, img_ids=[], cat_ids=[]):
        return self.coco.getImgIds(img_ids, cat_ids)
    ...
    def load_annotations(self, ann_file):
        ...
        for i in self.img_ids:
            info = self.load_imgs([i])[0]
            info['filename'] = info['file_name']

            ann = self.get_ann_info(info)
            # info.update(ann)
            ann_ids = self.get_ann_ids(img_ids=[i])
            # ann['ann_id'], ann['true_bboxes'] = ann_ids, ann['bboxes']
            info['ann'] = ann
    ...
    def get_ann_info(self, info):
        ...
        ann_info = self.load_anns(ann_ids)
    ...
    def _parse_ann_info(self, img_info, ann_info):
        ...
        ann = dict(
            bboxes=gt_bboxes, true_bboxes=gt_bboxes, ann_id=gt_ann_ids,
            labels=gt_labels,
